...
datos <- read_excel("C:\\Users\\rpm0a\\OneDrive\\Documentos\\RepTemplates\\ZonasMetro\\Bases temporales\\Bases Agrupadas\\Bases Largas Agrupadas Finales\\BLzm04a_final.xlsx")

# Seleccionar solo las columnas relevantes
df <- datos %>% 
  select(cvegeo, CVE_ZM, NOM_ZM, cv_mun, nom_mun, cv_ent, nom_ent, sect, 
...
df_wide <- df %>% 
  pivot_wider(names_from = sect, values_from = c("ue", "af", "fb", "pb", "po", "re", "va", 
              ...
              names_glue = "{.value}.{sect}_04")

# Unir valores únicos de cvegeo con datos agrupados por cve_sub

zma04 <- left_join(df_distinct, df_wide, by = "cvegeo")
View(zm19)

# Guardar como archivo xlsx

library(openxlsx)
write.xlsx(zma04, "zma04.xlsx")
